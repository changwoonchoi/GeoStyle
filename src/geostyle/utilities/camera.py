...
def persp_proj(fov_x=45, ar=1, near=1.0, far=50.0, flip_x=False):
    """
...
def get_camera_params(elev_angle, azim_angle, distance, resolution, fov=60, look_at=[0, 0, 0], up=[0, -1, 0], flip_x=False):
    ...
    proj_mtx = persp_proj(fov_x=fov, flip_x=flip_x)
...
class CameraBatch(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.aug_bkg:
            ...
            if self.bkg_color == "white":
                bkgs = torch.ones(self.res, self.res, 3)  # just use white background
            elif self.bkg_color == "gray":
                bkgs = torch.ones(self.res, self.res, 3) * 0.7
            elif self.bkg_color == "black":
...
class FixedDistFOVCameraBatch(torch.utils.data.Dataset):
    '''
    Distance and fov fixed, randomly sample azim and elev from the given range
    '''
    def __init__(
        self,
        image_resolution,
        ...
        self.elev_min = elevation_min
        self.elev_max = elevation_max

        self.azim_min = azimuth_min
        self.azim_max = azimuth_max
    ...
    def __getitem__(self, index):
        elev = np.radians(np.random.uniform(self.elev_min, self.elev_max))
        azim = np.radians(np.random.uniform(self.azim_min, self.azim_max))
