...
def shade(
    ...
    if render_depth:
        batch_size = gb_depth.shape[0]
        gb_depth_min = torch.min(gb_depth.view(batch_size, -1), dim=-1, keepdim=True)[0].view(batch_size, 1, 1, 1)
        gb_depth_max = torch.max(gb_depth.view(batch_size, -1), dim=-1, keepdim=True)[0].view(batch_size, 1, 1, 1)
        new_min = 0.05
        new_max = 0.4
        normalized_depth = (gb_depth - gb_depth_min) / (gb_depth_max - gb_depth_min) * (new_max - new_min) + new_min
        depth_color = normalized_depth.repeat(1, 1, 1, 3)
        shaded_depth_col = 1 - depth_color
        depth_out = torch.cat((shaded_depth_col, alpha), dim=-1)
    ...
    buffers = {
        ...
        'depth'     : depth_out if render_depth else None,
    }
    return buffers
...
def render_layer(
    ...
    depth_ = v_pos_clip[0, :, 2:3] / v_pos_clip[0, :, 3:4]
    depth = torch.tensor([[[(z_val/1)] for z_val in depth_.squeeze()]], dtype=torch.float32, device='cuda')
    gb_depth, _ = interpolate(
        depth, rast_out_s, mesh.t_pos_idx.int()
    )
    ...
    buffers = shade(gb_pos, gb_geometric_normal, gb_normal, gb_tangent, gb_texc, gb_texc_deriv, 
        view_pos, lgt, mesh.material, bsdf, render_depth, gb_depth)
...
def render_layer_with_vertex_color(
        rast,
        rast_deriv,
        mesh,
        view_pos,
    ...
    # Interpolate world space position
    color, _ = interpolate(vertex_color[None, ...], rast_out_s, mesh.t_pos_idx.int())
...
def render_mesh(
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), full_res) as peeler:
        for _ in range(num_layers):
            ...
            layers += [(render_layer(rast, db, mesh, view_pos, lgt, resolution, spp, msaa, bsdf, render_depth, v_pos_clip), rast)]
    ...
    # Setup background for depth map
    if depth_background is not None:
        if spp > 1:
            depth_background = util.scale_img_nhwc(depth_background, full_res, mag='nearest', min='nearest')
        depth_background = torch.cat((depth_background, torch.zeros_like(depth_background[..., 0:1])), dim=-1)
    else:
        depth_background = torch.zeros(1, full_res[0], full_res[1], 4, dtype=torch.float32, device='cuda')
    ...
    for key in layers[0][0].keys():
        if key == 'shaded':
            ...
            if render_depth:
                accum = composite_buffer(key, layers, depth_background, True)
            else:
                continue
        else:
...
def render_mesh_with_vertex_color(
        ctx,
        mesh,
        mtx_in,
        view_pos,
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), full_res) as peeler:
        for _ in range(num_layers):
            rast, db = peeler.rasterize_next_layer()
            vertex_shaded_color = render_layer_with_vertex_color(rast, db, mesh, view_pos, lgt, resolution, spp, msaa, bsdf, vertex_color)
