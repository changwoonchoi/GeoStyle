...
def write_obj(folder, mesh, save_material=True, fname=None, v_color=None):
    if fname is None:
        obj_file = os.path.join(folder, 'mesh.obj')
    else:
        obj_file = os.path.join(folder, fname)
    ...
    with open(obj_file, "w") as f:
        ...
        if v_color is not None:
            for v, color in zip(v_pos, v_color):
    ...
    if save_material:
        if fname is None:
            ...
            mtl_file = os.path.join(folder, fname.replace('.obj', '.mtl'))
