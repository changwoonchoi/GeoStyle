...
class EnvironmentLight(torch.nn.Module):
    ...
    def __init__(self, base, train_light=False):
    ...
    def shade(self, gb_pos, gb_normal, kd, ks, view_pos, specular=True):
        ...
        if specular:
            ...
            if not hasattr(self, '_FG_LUT'):
                self._FG_LUT = torch.as_tensor(np.fromfile('materials/irrmaps/bsdf_256_256.bin', dtype=np.float32).reshape(1, 256, 256, 2), dtype=torch.float32, device='cuda')
...
def _load_env_hdr(fn, scale=1.0, train_light=False, rotate=0):
    latlong_img = torch.tensor(util.load_image(fn), dtype=torch.float32, device='cuda')*scale
    if rotate != 0:
        rotate_int = int(latlong_img.shape[1] * rotate)
        latlong_img = torch.roll(latlong_img, rotate_int, dims=1)
    cubemap = util.latlong_to_cubemap(latlong_img, [512, 512])

    l = EnvironmentLight(cubemap, train_light)
    l.build_mips()

    return l

def load_env(fn, scale=1.0, train_light=False, rotate=0):
    if os.path.splitext(fn)[1].lower() == ".hdr":
        return _load_env_hdr(fn, scale, train_light, rotate)
    else:
