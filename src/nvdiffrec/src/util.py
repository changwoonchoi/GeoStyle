...
import imageio
from nvdiffrec.src import texture
...
def prepare_trainable_material(cfg, mlp=False, init_mat=None, optimize_mat=False):
    kd_min, kd_max = torch.tensor(cfg.kd_min, dtype=torch.float32, device='cuda'), torch.tensor(cfg.kd_max, dtype=torch.float32, device='cuda')
    ks_min, ks_max = torch.tensor(cfg.ks_min, dtype=torch.float32, device='cuda'), torch.tensor(cfg.ks_max, dtype=torch.float32, device='cuda')
    nrm_min, nrm_max = torch.tensor(cfg.nrm_min, dtype=torch.float32, device='cuda'), torch.tensor(cfg.nrm_max, dtype=torch.float32, device='cuda')
    if mlp:
        raise NotImplementedError('MLP texture is not implemented')
    else:
        # Setup Kd (albedo) and Ks (x, roughness, metalness) textures
        if cfg.random_textures or init_mat is None:
            num_channels = 4 if cfg.layers > 1 else 3
            kd_init = torch.rand(size=cfg.texture_res + [num_channels], device='cuda') * (kd_max - kd_min)[None, None, 0:num_channels] + kd_min[None, None, 0:num_channels]
            kd_map_opt = texture.create_trainable(kd_init , cfg.texture_res, True, [kd_min, kd_max])

            ksR = np.random.uniform(size=cfg.texture_res + [1], low=0.0, high=0.01)
            ksG = np.random.uniform(size=cfg.texture_res + [1], low=ks_min[1].cpu(), high=ks_max[1].cpu())
            ksB = np.random.uniform(size=cfg.texture_res + [1], low=ks_min[2].cpu(), high=ks_max[2].cpu())

            ks_map_opt = texture.create_trainable(np.concatenate((ksR, ksG, ksB), axis=2), cfg.texture_res, not cfg.custom_mip, [ks_min, ks_max])
        else:
            kd_map_opt = texture.create_trainable(init_mat['kd'], cfg.texture_res, True, [kd_min, kd_max])
            ks_map_opt = texture.create_trainable(init_mat['ks'], cfg.texture_res, True, [ks_min, ks_max])

        # Setup normal map
        if cfg.random_textures or init_mat is None or 'normal' not in init_mat:
            normal_map_opt = texture.create_trainable(np.array([0, 0, 1]), cfg.texture_res, True, [nrm_min, nrm_max])
        else:
            normal_map_opt = texture.create_trainable(init_mat['normal'], cfg.texture_res, True, [nrm_min, nrm_max])
        
        if not optimize_mat:
            kd_map_opt.data.requires_grad = False
            ks_map_opt.data.requires_grad = False
            normal_map_opt.data.requires_grad = False
